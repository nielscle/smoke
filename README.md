# Reclaim

Een mobiele webapp die zichtbaar maakt hoe het lichaam herstelt na stoppen met roken.

## Openen
Open `index.html` direct in een browser. Voor volledige offline-installatie via de service worker is een lokale webserver nodig:

```bash
python3 -m http.server 8080
```

Open daarna `http://localhost:8080`.

## Nieuwe versie
- 51 gezondheidsmijlpalen van 20 minuten tot 15 jaar
- Dagelijkse mijlpalen in de eerste twee weken
- Wekelijkse mijlpalen tot drie maanden
- Maandelijkse mijlpalen tot één jaar
- Jaarlijkse mijlpalen tot vijftien jaar
- 10 verdiepende gezondheidsgebieden
- Uitgebreidere voortgangsbalken per gezondheidsgebied
- Craving-interventies, check-ins, besparing, spaardoel en uitglijders
- Offline ondersteuning als PWA
- Lokale opslag in de browser

## Medische inhoud
De app geeft algemene gezondheidsinformatie, geen diagnose of persoonlijke risicoberekening. De tijdlijnen zijn algemene gemiddelden en verschillen per persoon. Voor publicatie in een appstore is inhoudelijke controle door een arts of stoppen-met-rokenprofessional aan te raden.
